...
class SteamTurbine:
    ...
    def check_points(self, point):
        ...
        for k in self.coef_list:
            ...
            relative_difference = (
                self._relative_difference(coef_guaranteed, coef_test) * 100
            )
        ...
        def objective_func(x):
            ...
            sum = np.sum(
                self._relative_difference(
                    np.array(guaranteed_coefs), np.array(test_coefs)
                )
            )
            return sum
    ...
    def _mount_point(self, parameters, x):
        ...
        point = Point(inlet=State(**inlet_dict), eff=self.guaranteed.eff, **test_dict)
    ...
    def print_compare(self, point=None):
        ...
        titles = [
            ...
            "Volume Ratio",
            "Flow Coefficient - Inlet",
            "Flow Coefficient - Discharge",
        ]
        for k, title in enumerate(titles):
            guaranteed_txt = (
                f"Guaranteed:  {getattr(self.guaranteed, self.coef_list[k]).m:.3f}"
            )
